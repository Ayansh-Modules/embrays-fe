...
function Drawer({ children, isOpen, setIsOpen }) {
  return (
    <main
      className={
        " fixed overflow-hidden z-10 bg-gray-700 bg-opacity-90 inset-0 transform ease-in-out " +
        (isOpen
    ...
    >
      ...
      >
        <article className="relative w-screen max-w-lg pb-10 flex flex-col space-y-6 overflow-y-scroll h-full">
          ...
          <GiCancel onClick={()=>{setIsOpen(false)}} className="m-[20px] h-[20px] w-[20px] text-white "/>
          {children}
        </article>
      </section>
      <section
